    ...
    <div className="rounded-2xl sm:rounded-3xl border border-slate-200 bg-white p-3 sm:p-6 lg:p-8 shadow-soft space-y-3.5 sm:space-y-7 w-full overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 sm:gap-3 border-b border-slate-100 pb-3 sm:pb-5">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[10px] sm:text-xs font-bold text-slate-600 bg-slate-100 px-2.5 py-1 rounded-full border border-slate-200">
      ...
      <div className="relative pl-1 sm:pl-3">
        ...
        <div className="relative z-10 flex items-start gap-2.5 sm:gap-3">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 p-3 sm:p-4 rounded-2xl bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white shadow-soft-lg border border-slate-800 w-full sm:max-w-xl">
            <div className="flex items-center gap-2.5 sm:gap-3">
              <div className="h-8 w-8 sm:h-10 sm:w-10 rounded-xl bg-white/10 backdrop-blur-md flex items-center justify-center border border-white/20 shrink-0">
                <FolderGit2 className="h-4 w-4 sm:h-5 sm:w-5 text-brand-300" />
              </div>
              <div className="min-w-0">
                ...
                <h4 className="text-xs sm:text-base font-extrabold tracking-tight truncate">
                  {project?.name}
                </h4>
        ...
        {/* Tree Empty Fallback */}
        {developers.length === 0 ? (
          <div className="ml-5 sm:ml-7 mt-3 p-4 rounded-xl border border-dashed border-slate-300 text-xs text-slate-500">
            No developers assigned to this workspace yet.
          </div>
        ) : (
          /* Vertical Trunk Pipe from Project Root */
          <div className="relative ml-4 sm:ml-5 border-l-2 border-slate-300 pt-3 pb-2 space-y-4 sm:space-y-6">
            {developers.map((dev) => {
              ...
              return (
                <div key={dev._id} className="relative pl-3.5 sm:pl-7 group/branch">
                  {/* Horizontal Branch Pipe to Developer Node */}
                  <div className="absolute left-0 top-4 sm:top-5 w-3.5 sm:w-7 h-0.5 bg-slate-300" />
                  <div className="absolute left-[-5px] top-[13px] sm:top-[17px] h-2.5 w-2.5 rounded-full bg-indigo-500 ring-4 ring-white" />

                  {/* 2. DEVELOPER BRANCH NODE */}
                  <div
                    onClick={() => toggleCollapse(dev._id)}
                    className={`flex items-center justify-between gap-2 p-2 sm:p-3 rounded-xl sm:rounded-2xl border transition-all duration-200 cursor-pointer w-full sm:max-w-lg shadow-soft-xs ${
                      isMe
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <span className="text-[9px] sm:text-[10px] font-bold font-mono px-2 py-0.5 rounded-full bg-white border border-slate-200 text-slate-700 shadow-2xs">
                        {completedCount}/{devPhases.length}
                      </span>
                  ...
                  {!isCollapsed && (
                    <div className="relative ml-2 sm:ml-5 mt-2 border-l-2 border-indigo-200 pt-1 pb-1 space-y-2">
                      {/* Top Insert Button (Insert at Position #1) */}
                      {onInsertPhase && (!currentUserId || isMe) && (
                        <div className="relative pl-3.5 sm:pl-7 pb-1 pt-0.5">
                          <button
                            type="button"
                            onClick={(e) => {
                              e.stopPropagation();
                            ...
                            className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-bold text-brand-700 bg-brand-50/90 hover:bg-brand-100 border border-dashed border-brand-300 hover:border-brand-400 transition-all shadow-2xs cursor-pointer active:scale-95"
                          >
                      ...
                      {devPhases.length === 0 ? (
                        <div className="relative pl-3.5 sm:pl-6 py-2 text-[11px] text-slate-400 italic font-medium">
                        ...
                        devPhases.map((phase, idx) => (
                          <React.Fragment key={phase._id}>
                            <div className="relative pl-3.5 sm:pl-7 group/phase">
                              ...
                              <div
                                ...
                                className={`w-full sm:max-w-lg flex flex-col p-2.5 sm:p-3 rounded-xl sm:rounded-2xl border-2 transition-all duration-200 cursor-pointer shadow-soft-xs ${
                                  phase.completed
                              ...
                              >
                                <div className="flex items-start justify-between gap-2">
                                  ...
                                  <div className="flex items-center gap-1.5 shrink-0">
                                    ...
                                    {onMovePhase && (!currentUserId || isMe) && (
                                      <div className="flex items-center rounded-lg border border-slate-200 bg-slate-50/80 p-0.5 shadow-2xs">
                                        <button
                                          ...
                                          title="Move Phase Up (▲)"
                                          className="p-1 rounded text-slate-500 hover:text-brand-600 hover:bg-white disabled:opacity-25 disabled:hover:bg-transparent disabled:hover:text-slate-500 transition-all cursor-pointer active:scale-95"
                                        >
                                    ...
                                    {/* Status Pill */}
                                    {phase.completed ? (
                                      <span className="inline-flex items-center gap-0.5 sm:gap-1 text-[9px] sm:text-[11px] font-bold text-emerald-800 bg-emerald-100/90 border border-emerald-300 px-1.5 py-0.5 sm:px-2 sm:py-0.5 rounded-full shadow-soft-xs">
                                        <CheckCircle2 className="h-2.5 w-2.5 sm:h-3 sm:w-3 text-emerald-600" />
                                        Done
                                      </span>
                                    ) : (
                                      <span className="inline-flex items-center gap-0.5 sm:gap-1 text-[9px] sm:text-[11px] font-semibold text-amber-800 bg-amber-50 border border-amber-200 px-1.5 py-0.5 sm:px-2 sm:py-0.5 rounded-full shadow-soft-xs">
                            ...
                            {onInsertPhase && (!currentUserId || isMe) && (
                              <div className="relative pl-3.5 sm:pl-7 py-0.5 group/insert">
                                <button
                                  type="button"
                                  onClick={(e) => {
                                    e.stopPropagation();
                                  ...
                                  className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-bold text-slate-400 hover:text-brand-700 bg-white hover:bg-brand-50 border border-slate-200 hover:border-brand-300 transition-all shadow-2xs cursor-pointer hover:scale-105 active:scale-95"
                                >
